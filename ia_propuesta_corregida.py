"""Version corregida de la propuesta de IA."""
import asyncio
...
class SolicitudPuntuacion(BaseModel):
    ...
    correo_analista: str = Field(pattern=r"^[A-Za-z0-9_.+-]+@[A-Za-z0-9-]+\.[A-Za-z]{2,63}$")
...
async def evaluar_lote(solicitudes: list[SolicitudPuntuacion]) -> list[float]:
    return await asyncio.gather(*(_puntuar(solicitud) for solicitud in solicitudes))
